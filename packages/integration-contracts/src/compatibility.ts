...
function compareSchema(
  ...
  for (const key of Object.keys(nextProperties)) {
    ...
    findings.push({
      path: `${path}.${key}`,
      rule: required ? 'required property added' : 'optional property added',
      breaking,
  ...
  for (const key of previousRequired)
    if (!nextRequired.has(key))
      findings.push({
        path: `${path}.${key}`,
        rule: 'requiredness relaxed',
        breaking: false,
        severity: 'info',
      });
  for (const key of nextRequired)
    if (!previousRequired.has(key) && key in previousProperties)
      findings.push({
        path: `${path}.${key}`,
        rule: 'requiredness tightened',
        breaking: true,
        severity: 'error',
      });
  ...
  if (previous.items && next.items)
    compareSchema(previous.items, next.items, `${path}[]`, profile, findings, seen);
  for (const keyword of ['allOf', 'anyOf', 'oneOf']) {
    ...
    if (oldChildren && newChildren && oldChildren.length === newChildren.length)
      ...
      findings.push({
        path,
        rule: `${keyword} composition changed`,
        breaking: true,
        severity: 'error',
      });
  }
...
function compareUpperBound(
  ...
  if (next !== undefined && (previous === undefined || next > previous))
    findings.push({ path, rule, breaking: true, severity: 'error' });
}
...
function sameJson(left: unknown, right: unknown): boolean {
  return (
    JSON.stringify(left, Object.keys((left as object) ?? {}).sort()) ===
    JSON.stringify(right, Object.keys((right as object) ?? {}).sort())
  );
}
