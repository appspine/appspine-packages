...
| --- | --- | --- | --- |
| REST（8 個 App 全部 API） | `JwtAuthGuard` → `AuthGuard('jwt-oidc')` → [`strategies/oidc.strategy.ts`](../../packages/oidc-auth/src/strategies/oidc.strategy.ts) | **passport-jwt 自己** | 否 |
| WebSocket（僅 chat） | ``chat.gateway.ts`` → `verifyJwtToken()` | [`verifyOidcSignature()`](../../packages/oidc-auth/src/jwt-verifier.service.ts) | 是 |
...
- [`jwt-verifier.service.ts`](../../packages/oidc-auth/src/jwt-verifier.service.ts)
- [`strategies/oidc.strategy.ts`](../../packages/oidc-auth/src/strategies/oidc.strategy.ts)
