...
try {
  if (command === 'index') await indexContracts(args.write === true, args.check === true);
    ...
    validateContracts(args.path ? resolve(root, args.path) : undefined);
  else if (command === 'init') initContract(args);
  else if (command === 'diff' || command === 'check-compatibility')
    compareContracts(args, command === 'diff');
...
function parseArgs(values) {
  ...
  for (let index = 0; index < values.length; index += 1) {
    ...
    result[key] =
      values[index + 1]?.startsWith('--') || values[index + 1] === undefined
        ? true
        : values[++index];
  }
  return result;
...
function discoverContracts() {
  ...
  return result.sort((a, b) =>
    `${a.contractId}@${a.version}`.localeCompare(`${b.contractId}@${b.version}`),
  );
}
...
function collectArtifacts(versionRoot, markdown) {
  ...
  return Object.fromEntries(
    files
      .sort()
      .map((path) => [
        relative(versionRoot, path).split(sep).join('/'),
...
function makeManifest(input) {
  const unsigned = {
    ...
    artifacts: Object.fromEntries(Object.entries(input.artifacts).sort()),
  };
...
async function indexContracts(write, check) {
  ...
  if (check) {
    const currentJson = existsSync(join(contractsRoot, 'index.json'))
      ? readFileSync(join(contractsRoot, 'index.json'), 'utf8')
      : '';
    const currentMarkdown = existsSync(join(contractsRoot, 'index.md'))
      ? readFileSync(join(contractsRoot, 'index.md'), 'utf8')
      : '';
    const rootOnly = args['root-only'] === true;
    if (rootOnly) {
      let currentIndex;
      try {
        currentIndex = JSON.parse(currentJson);
      } catch {
        currentIndex = undefined;
      }
      if (
        canonicalJson(withoutLocalUsage(currentIndex)) !==
          canonicalJson(withoutLocalUsage(index)) ||
        stripUsageColumn(currentMarkdown) !== stripUsageColumn(markdown)
      )
        throw new Error(
          'contract indexes are stale; run node scripts/contract-cli.mjs index --write',
        );
...
function collectLocalUsage() {
  ...
  if (missingAny && usage.size === 0) {
    console.warn(
      '[contract-cli] Warning: Local consumer app repositories not found; skipping local usage scan.',
    );
  }
  return usage;
}
...
function withoutLocalUsage(value) {
  ...
  return Object.fromEntries(
    Object.entries(value)
      .filter(([key]) => key !== 'usedByApps')
      .map(([key, child]) => [key, withoutLocalUsage(child)]),
  );
}
...
function stripUsageColumn(markdown) {
  return markdown
    .split('\n')
    .map((line) => {
      if (!line.startsWith('|') || line.startsWith('| ---') || line.startsWith('| Contract'))
        return line;
...
function supportedMajorVersions(contracts, contractId) {
  return contracts
    .filter(
      (contract) =>
        contract.contractId === contractId && ['approved', 'deprecated'].includes(contract.status),
    )
...
function compareVersions(left, right) {
  return left
    .split('.')
    .map(Number)
    .reduce((result, value, index) => result || value - Number(right.split('.')[index]), 0);
}
...
function validateContracts(path) {
  ...
  if (path) {
    ...
    if (!target.startsWith(`${contractsRoot}${sep}`) && target !== contractsRoot)
      throw new Error('Validation path must stay inside knowledge/contracts');
  }
  if (invalid.length) throw new Error(invalid.join('\n'));
...
function validateDiscovered(contracts) {
  ...
  for (const contract of contracts) {
    ...
    if (fm.type !== 'integration-contract')
      errors.push(`${key}: type must be integration-contract`);
    if (!/^\d+\.\d+\.\d+$/u.test(contract.version)) errors.push(`${key}: invalid SemVer`);
    if (fm.contract_id !== contract.contractId || fm.version !== contract.version)
      errors.push(`${key}: directory and frontmatter identity differ`);
    if (!['command', 'query', 'event'].includes(fm.interaction))
      errors.push(`${key}: interaction is required`);
    if (!['draft', 'review', 'approved', 'deprecated'].includes(fm.status))
      errors.push(`${key}: invalid status`);
    for (const heading of [
      ...
      if (!new RegExp(`^## ${escapeRegex(heading)}$`, 'mu').test(content))
        errors.push(`${key}: missing heading "${heading}"`);
    }
    if (contract.kind === 'capability') {
      if (['command', 'query'].includes(fm.interaction)) {
        if (typeof fm.provider !== 'string' || !fm.provider)
          errors.push(`${key}: ${fm.interaction} capability requires provider`);
        if (!Array.isArray(fm.callers) || fm.callers.length === 0)
          errors.push(`${key}: ${fm.interaction} capability requires callers`);
        if (fm.transport !== 'http')
          errors.push(`${key}: ${fm.interaction} capability must use HTTP transport`);
      } else {
        if (typeof fm.producer !== 'string' || !fm.producer)
          errors.push(`${key}: event capability requires producer`);
        if (!Array.isArray(fm.consumers) || fm.consumers.length === 0)
          errors.push(`${key}: event capability requires consumers`);
        if (!['webhook', 'message-broker'].includes(fm.transport))
          errors.push(`${key}: event capability must use webhook or message-broker transport`);
      }
    }
    if (contract.kind === 'binding') {
      if (!fm.capability_ref || !fm.source_app || !fm.destination_app)
        errors.push(`${key}: capability_ref, source_app and destination_app are required`);
      if (fm.source_app === fm.destination_app)
        errors.push(`${key}: source_app and destination_app must differ`);
      if (typeof fm.destination_key !== 'string' || !fm.destination_key)
        errors.push(`${key}: destination_key is required`);
      if (
        !fm.authentication ||
        typeof fm.authentication !== 'object' ||
        Array.isArray(fm.authentication)
      )
        errors.push(`${key}: authentication policy is required`);
      if (!fm.retry || typeof fm.retry !== 'object' || Array.isArray(fm.retry))
        errors.push(`${key}: retry policy is required`);
      else {
        if (!Number.isInteger(fm.retry.timeout_ms) || fm.retry.timeout_ms <= 0)
          errors.push(`${key}: retry.timeout_ms must be a positive integer`);
        if (!Number.isInteger(fm.retry.max_attempts) || fm.retry.max_attempts <= 0)
          errors.push(`${key}: retry.max_attempts must be a positive integer`);
      }
    }
      ...
      if (artifact.endsWith('.json')) {
        try {
          ...
          if (artifact.endsWith('.schema.json'))
            errors.push(...validateJsonSchemaDocument(parsed, `${key}:${artifact}`));
        } catch (error) {
          errors.push(
            `${key}: invalid JSON artifact ${artifact}: ${error instanceof Error ? error.message : String(error)}`,
          );
        }
      }
      if (artifact.endsWith('.yaml') || artifact.endsWith('.yml'))
        errors.push(
          ...validateOpenApiDocument(readFileSync(artifactPath, 'utf8'), `${key}:${artifact}`),
        );
    }
  }
  ...
  if (existsSync(lockPath)) {
    ...
    for (const contract of contracts.filter((candidate) => candidate.status === 'approved')) {
      ...
      if (!expected)
        errors.push(`${contract.contractId}@${contract.version}: approved digest lock is missing`);
      else if (expected !== contract.digest)
        errors.push(
          `${contract.contractId}@${contract.version}: approved contract is immutable and its digest changed`,
        );
    }
  }
  const capabilities = new Map(
    contracts
      .filter((contract) => contract.kind === 'capability')
      .map((contract) => [`${contract.contractId}@${contract.version}`, contract]),
  );
  for (const binding of contracts.filter((contract) => contract.kind === 'binding')) {
    ...
    if (ref && !capability)
      errors.push(`${binding.contractId}@${binding.version}: capability_ref not found`);
    else if (ref && capability) {
      if (ref.digest !== capability.digest)
        errors.push(
          `${binding.contractId}@${binding.version}: capability_ref digest does not match canonical capability`,
        );
      if (binding.interaction !== capability.interaction)
        errors.push(
          `${binding.contractId}@${binding.version}: interaction must match referenced capability`,
        );
      if (['command', 'query'].includes(capability.interaction)) {
        if (!capability.frontmatter.callers?.includes(binding.frontmatter.source_app))
          errors.push(
            `${binding.contractId}@${binding.version}: source_app must be a declared caller`,
          );
        if (binding.frontmatter.destination_app !== capability.frontmatter.provider)
          errors.push(
            `${binding.contractId}@${binding.version}: destination_app must be the declared provider`,
          );
      } else {
        if (binding.frontmatter.source_app !== capability.frontmatter.producer)
          errors.push(
            `${binding.contractId}@${binding.version}: source_app must be the declared producer`,
          );
        if (!capability.frontmatter.consumers?.includes(binding.frontmatter.destination_app))
          errors.push(
            `${binding.contractId}@${binding.version}: destination_app must be a declared consumer`,
          );
      }
    }
  }
...
function validateContractFrontmatter(fm, key) {
  const errors = validateMetaSchemaValue(fm, META_SCHEMA, '$', META_SCHEMA).map(
    (message) => `${key}: frontmatter ${message}`,
  );
  const required = [
    'type',
    'contract_kind',
  ...
  for (const field of required)
    if (fm[field] === undefined || fm[field] === null || fm[field] === '')
      errors.push(`${key}: frontmatter ${field} is required`);
  if (fm.contract_kind === 'capability' && !fm.provider && !fm.producer)
    errors.push(`${key}: capability requires provider or producer`);
  if (
    fm.contract_kind === 'binding' &&
    (!fm.capability_ref || !fm.source_app || !fm.destination_app)
  )
    errors.push(`${key}: binding requires capability_ref, source_app and destination_app`);
  if (
    fm.required_reviewers !== undefined &&
    (!Array.isArray(fm.required_reviewers) || fm.required_reviewers.length === 0)
  )
    errors.push(`${key}: required_reviewers must be a non-empty array`);
  if (fm.status === 'approved' && fm.implementation_status === 'disabled')
    errors.push(`${key}: approved contract cannot be disabled`);
  return errors;
}
...
function validateMetaSchemaValue(value, schema, path, rootSchema) {
  ...
  if (schema.$ref) {
    ...
    return resolved
      ? validateMetaSchemaValue(value, resolved, path, rootSchema)
  ...
  if (schema.type && !metaTypeMatches(value, schema.type))
    errors.push(
      `${path}: must be ${Array.isArray(schema.type) ? schema.type.join(' or ') : schema.type}`,
    );
  ...
  if (value && typeof value === 'object' && !Array.isArray(value)) {
    ...
    for (const [key, child] of Object.entries(schema.properties ?? {}))
      if (key in record)
        errors.push(...validateMetaSchemaValue(record[key], child, `${path}.${key}`, rootSchema));
  ...
  for (const child of schema.allOf ?? [])
    errors.push(...validateMetaSchemaValue(value, child, path, rootSchema));
  if (
    schema.anyOf &&
    !schema.anyOf.some(
      (child) => validateMetaSchemaValue(value, child, path, rootSchema).length === 0,
    )
  )
...
function validateJsonSchemaDocument(schema, label, path = '$', seen = new Set()) {
  ...
  if (!schema || typeof schema !== 'object' || Array.isArray(schema))
    return [`${label}:${path}: schema node must be an object`];
  if (seen.has(schema)) return errors;
  seen.add(schema);
  for (const keyword of Object.keys(schema)) {
    if (!CLI_SCHEMA_KEYWORDS.has(keyword))
      errors.push(`${label}:${path}: unsupported schema keyword ${keyword}`);
    if (keyword.startsWith('x-appspine-') && keyword !== 'x-appspine-data-classification')
      errors.push(`${label}:${path}: unknown Appspine schema keyword ${keyword}`);
  }
  const classification = schema['x-appspine-data-classification'];
  if (classification === 'SECRET')
    errors.push(`${label}:${path}: SECRET classification is forbidden`);
  if (
    classification !== undefined &&
    !['PUBLIC', 'INTERNAL', 'PERSONAL', 'SENSITIVE'].includes(classification)
  )
    errors.push(`${label}:${path}: invalid data classification`);
  const children = [];
  for (const [key, child] of Object.entries(schema.properties ?? {}))
    children.push([`${path}.${key}`, child]);
  for (const [key, child] of Object.entries(schema.patternProperties ?? {}))
    children.push([`${path}.{${key}}`, child]);
  for (const [key, child] of Object.entries(schema.$defs ?? {}))
    children.push([`${path}.$defs.${key}`, child]);
  for (const [index, child] of schema.items ? [['items', schema.items]] : [])
    children.push([`${path}.${index}`, child]);
  for (const keyword of ['allOf', 'anyOf', 'oneOf'])
    for (const [index, child] of (schema[keyword] ?? []).entries())
      children.push([`${path}.${keyword}[${index}]`, child]);
  for (const keyword of ['if', 'then', 'else'])
    if (schema[keyword]) children.push([`${path}.${keyword}`, schema[keyword]]);
  ...
  if (
    ...
    errors.push(`${label}:${path}: every schema leaf requires x-appspine-data-classification`);
  for (const [childPath, child] of children)
    errors.push(...validateJsonSchemaDocument(child, label, childPath, seen));
  return errors;
}

function validateOpenApiDocument(content, label) {
  const errors = [];
  if (!/^openapi:\s*3\.1\.\d+\s*$/mu.test(content))
    errors.push(`${label}: OpenAPI document must declare 3.1.x`);
  if (!/^info:\s*$/mu.test(content) && !/^info:\s*\{/mu.test(content))
    errors.push(`${label}: OpenAPI info is required`);
  if (!/^paths:\s*$/mu.test(content) && !/^paths:\s*\{/mu.test(content))
    errors.push(`${label}: OpenAPI paths are required`);
  for (const match of content.matchAll(/\$ref:\s*(['"]?)([^'"},\s]+)/gu))
    if (!match[2].startsWith('#'))
      errors.push(`${label}: external OpenAPI references are not allowed`);
  for (const status of content.matchAll(/^\s{8,}'?(\d{3})'?\s*:/gmu))
    if (
      ![
        '200',
        '201',
      ...
      errors.push(`${label}: unsupported HTTP response status ${status[1]}`);
  return errors;
}
...
function initContract(options) {
  ...
  if (!['capability', 'binding'].includes(kind) || !contractId || !/^\d+\.\d+\.\d+$/u.test(version))
    throw new Error('init requires --kind capability|binding --id <id> --version <semver>');
  const directory = join(
    contractsRoot,
    kind === 'capability' ? 'capabilities' : 'bindings',
    contractId,
    'versions',
    version,
  );
  ...
  if (existsSync(file)) {
    ...
    if (existing.status === 'approved' || existing.status === 'deprecated')
      throw new Error(`cannot overwrite immutable ${existing.status} contract: ${file}`);
    if (options.force !== true) throw new Error(`already exists: ${file}`);
  }
  writeFileSync(
    file,
    kind === 'capability'
      ? capabilitySkeleton(contractId, version)
      : bindingSkeleton(contractId, version),
  );
  writeFileSync(
    join(directory, 'schemas', 'payload.schema.json'),
    `${JSON.stringify({ $schema: 'https://json-schema.org/draft/2020-12/schema', type: 'object', additionalProperties: false }, null, 2)}\n`,
  );
...
function compareContracts(options, printDiff) {
  ...
  if (!from || !to)
    throw new Error('diff requires --from <contract-id@version> and --to <contract-id@version>');
  const profile = options.profile ?? 'provider-compatible';
  if (!['strict', 'tolerant-reader', 'provider-compatible'].includes(profile))
    throw new Error(`unsupported compatibility profile: ${profile}`);
  const findings = compareSchemas(from, to, profile);
  const result = {
    from: `${from.contractId}@${from.version}`,
    to: `${to.contractId}@${to.version}`,
    profile,
    compatible: findings.every((finding) => !finding.breaking),
    requiredVersionBump: findings.some((finding) => finding.breaking)
      ? 'major'
      : findings.length
        ? 'minor'
        : 'none',
...
function compareSchemas(from, to, profile) {
  ...
  for (const artifact of artifactNames) {
    ...
    if (!fromSchema || !toSchema) {
      allFindings.push({
        path: artifact,
        breaking: Boolean(fromSchema),
        severity: fromSchema ? 'error' : 'info',
        rule: fromSchema ? 'schema artifact removed' : 'schema artifact added',
      });
      continue;
    }
    allFindings.push(
      ...compareSchemaPair(fromSchema, toSchema, profile).map((finding) => ({
        ...finding,
        path: `${artifact}:${finding.path}`,
      })),
    );
  }
  return allFindings;
...
function compareSchemaPair(fromSchema, toSchema, profile) {
  try {
    const shared = createRequire(new URL(import.meta.url))(
      resolve(root, 'packages/integration-contracts/dist/index.js'),
    );
    if (shared.checkSchemaCompatibility)
      return shared.checkSchemaCompatibility(fromSchema ?? {}, toSchema ?? {}, profile);
  } catch (err) {
    console.warn(
      `[contract-cli] Warning: Failed to load @appspine/integration-contracts dist module (${err instanceof Error ? err.message : String(err)}). Falling back to basic compatibility checker.`,
    );
  }
  const findings = [];
  if (JSON.stringify(fromSchema?.type) !== JSON.stringify(toSchema?.type))
    findings.push({ path: '$', breaking: true, severity: 'error', rule: 'type changed' });
  for (const value of fromSchema?.enum ?? [])
    if (
      !(toSchema?.enum ?? []).some(
        (candidate) => JSON.stringify(candidate) === JSON.stringify(value),
      )
    )
      findings.push({ path: '$', breaking: true, severity: 'error', rule: 'enum value removed' });
  ...
  for (const field of fromRequired)
    if (!toRequired.has(field))
      findings.push({ path: `$.${field}`, breaking: false, rule: 'required field relaxed' });
  for (const field of toRequired)
    if (!fromRequired.has(field))
      findings.push({ path: `$.${field}`, breaking: true, rule: 'new required field' });
  const fromProperties = fromSchema?.properties ?? {};
  const toProperties = toSchema?.properties ?? {};
  for (const field of Object.keys(fromProperties))
    if (!(field in toProperties))
      findings.push({
        path: `$.${field}`,
        breaking: profile !== 'tolerant-reader' || fromRequired.has(field),
        rule: 'property removed',
      });
  for (const field of Object.keys(toProperties))
    if (!(field in fromProperties) && (profile !== 'tolerant-reader' || toRequired.has(field)))
      findings.push({
        path: `$.${field}`,
        breaking: toRequired.has(field),
        rule: toRequired.has(field) ? 'new required field' : 'property added',
      });
  if (fromSchema?.additionalProperties === false && toSchema?.additionalProperties !== false)
    findings.push({
      path: '$',
      breaking: false,
      severity: 'info',
  ...
  if (fromSchema?.additionalProperties !== false && toSchema?.additionalProperties === false)
    findings.push({
      path: '$',
      breaking: profile !== 'tolerant-reader',
      severity: profile === 'tolerant-reader' ? 'warning' : 'error',
      rule: 'additional properties restricted',
    });
  return findings;
}
...
function syncViews(options) {
  ...
  const capability =
    contract.kind === 'binding' && contract.frontmatter.capability_ref
      ? discoverContracts().find(
          (candidate) =>
            candidate.kind === 'capability' &&
            candidate.contractId === contract.frontmatter.capability_ref.contract_id &&
            candidate.version === contract.frontmatter.capability_ref.version,
        )
      : undefined;
  const ref = {
    ...
    capability_ref: contract.frontmatter.capability_ref,
    generated_by: 'scripts/contract-cli.mjs',
  };
  ...
  const files = [
    { path: join(destination, 'contract-ref.json'), content: `${JSON.stringify(ref, null, 2)}\n` },
    {
      ...
      content: existsSync(implementationPath)
        ? readFileSync(implementationPath)
        : implementationTemplate(contract, target),
    },
  ];
  const artifactSource = capability ?? contract;
  for (const artifact of Object.keys(artifactSource.artifacts))
    if (artifact !== 'contract.md' && artifact !== 'binding.md')
      files.push({
        path: join(destination, artifact),
        content: readFileSync(join(root, dirname(artifactSource.source), artifact)),
      });
    ...
    JSON.stringify(
      {
        ...
        contract: ref,
        files: files.map((file) => relative(target, file.path).split(sep).join('/')),
      },
      null,
      2,
    ),
...
function generateRuntime(options) {
  ...
  const schemaContract =
    contract.kind === 'binding' && contract.frontmatter.capability_ref
      ? (findContract(
          `${contract.frontmatter.capability_ref.contract_id}@${contract.frontmatter.capability_ref.version}`,
        ) ?? contract)
      : contract;
    ...
    const stale = Object.entries(files)
      .filter(
        ([name, content]) =>
          ...
          readFileSync(join(directory, name), 'utf8') !== content,
      )
      .map(([name]) => name);
    if (stale.length > 0)
      throw new Error(
        `runtime artifacts are stale for ${contract.contractId}@${contract.version}: ${stale.join(', ')}`,
      );
    console.log(
      JSON.stringify(
        { mode: 'check', directory: relative(root, directory), files: Object.keys(files) },
        null,
        2,
      ),
    );
...
function findContract(value) {
  ...
  return discoverContracts().find(
    (contract) => contract.contractId === contractId && (!version || contract.version === version),
  );
}
...
function readSchemas(contract) {
  ...
  for (const artifact of Object.keys(contract.artifacts)
    ...
    result.set(
      artifact,
      JSON.parse(readFileSync(join(root, dirname(contract.source), artifact), 'utf8')),
    );
  }
  return result;
}
...
function typescriptType(schema) {
  ...
  if (schema.type === 'boolean' || schema.type === 'string' || schema.type === 'null')
    return schema.type;
  if (schema.anyOf || schema.oneOf)
    return (schema.anyOf ?? schema.oneOf).map(typescriptType).join(' | ');
