...
本 task 建立了多維度且完整的測試套件（5 個測試檔案，28 個單元與合約測試全數 PASS）：

### 2.1 Manifest 與 Plugin 合約測試（`plugin.spec.ts`）
...
### 2.2 Controller、Missing Optional Capability 與 Authorization Negative 測試（`meta.controller.spec.ts`）
...
### 2.3 Schema Drift 與 DMMF 動態自適應測試（`schema-drift.spec.ts`）
...
### 2.4 DMMF 核心解析與 I18n 測試（`meta.service.spec.ts` & `enum-i18n.test.ts`）
...
|---|---|---|
| 下游 9 個業務 App（`AppModule`） | `import { MetaModule } from "@appspine/metadata-schema";` 掛載於 `app.module.ts` | **零破壞性影響**。`MetaModule` 保持公開匯出並註冊 `MetaController`，下游 App 在 legacy 模式或 plugin 模式下開機與呼叫 `GET /metadata/schema` 完全相容。 |
...
| Calibration | 依 051 §11 執行嚴格校準以承接 G3 級別任務：<br>1. **架構合約維護**：在 `@appspine/plugin-api` 新增 `MetadataSchemaPort` 介面，於 `MetaModule` 綁定並匯出 `METADATA_SCHEMA` 穩定 token。<br>2. **消除 Concrete Guard 依賴**：重構 `MetaController`，以 `@appspine/plugin-host-nest` 的 `AppspineAuthGuard` 與中立 `MetadataScopeGuard` 取代直接依賴 `m2m-api-key` 具體 guard。<br>3. **嚴格安全 Fail-Closed 與 Optional Capability 處理**：實作 `MetadataScopeGuard`，在 `scope-matcher` 缺失時對 API Key 請求嚴格 fail-closed（403），對 JWT 正常放行。<br>4. **完整性驗證矩陣**：建立包含 manifest 合約、host resolution、boot catalog、missing optional capability、authorization negative、schema drift 自適應等 28 項測試，通過 monorepo 全套 platform gates。<br>5. **自我依賴審查**：完成自主 Dependency Audit 檢查並記錄審計表，供獨立 reviewer 覆核。 |
...
| Commit SHA | `aeb861de904bfa802c465017d26b1e772b506d3f` |
| Evidence | 28/28 unit tests in `@appspine/metadata-schema` pass; architecture check 22 packages (9 with manifest) 0 findings; generation gate 0 findings; build graph check 88 checks pass; changeset discipline pass; lint 0 errors; typecheck 0 errors |
