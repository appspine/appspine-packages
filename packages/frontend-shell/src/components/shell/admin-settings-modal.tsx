  ...
  render() {
    if (this.state.error) {
      return this.props.fallback ? (
        ...
        <DefaultErrorFallback retry={this.retry} />
      );
    }
...
function DefaultErrorFallback({ retry }: { readonly retry: () => void }) {
  return (
      ...
      <div className="w-full max-w-md rounded-xl border border-destructive/30 bg-destructive/5 p-6 shadow-xs">
        <div className="flex items-start gap-3">
          ...
          <div className="space-y-1">
            <h2 className="font-medium text-foreground">Unable to load this section</h2>
            <p className="text-sm leading-6 text-muted-foreground">
              Something went wrong while loading this administration section. Try again or choose
              another section from the navigation.
            </p>
          </div>
        </div>
    ...
    <Dialog open onOpenChange={(open) => !open && onClose()}>
      ...
      >
        ...
        <DialogDescription className="sr-only">
          Navigate administration settings and manage the selected section.
        </DialogDescription>
        ...
        <div className="flex min-h-0 min-w-0 flex-1 flex-col md:flex-row">
          ...
          <section className="flex min-h-0 min-w-0 flex-1 flex-col bg-background">
            <div className="flex h-11 shrink-0 items-center justify-end border-b px-3 sm:px-4">
              <DialogClose asChild>
                <Button variant="ghost" size="icon-sm" aria-label="Close administration settings">
                  <X aria-hidden="true" />
                </Button>
              </DialogClose>
            </div>
            <div className="min-h-0 flex-1 overflow-y-auto p-3 sm:p-4">
              <React.Suspense fallback={loadingFallback ?? <DefaultLoadingFallback />}>
                <ModalErrorBoundary fallback={errorFallback}>{children}</ModalErrorBoundary>
              </React.Suspense>
            </div>
          </section>
        </div>
