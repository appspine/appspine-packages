#!/usr/bin/env node
/**
 * PL5-06 — Chat Wave A Canary Plugin Mode Verification Script
 *
 * Validates:
 *  1. Builds and packs all 22 monorepo packages into clean canary tarballs.
 *  2. Installs canary tarballs into chat via concrete tarball overrides.
 ...
 * 10. Validates WebSocket lifecycle and graceful OnApplicationShutdown hook.
 */
...
const chatRoot = path.resolve('D:/Source/Private/appspine/chat');
...
async function main() {
  console.log('===============================================================');
  console.log('PL5-06: Chat Wave A Plugin Mode End-to-End Verification');
  console.log('===============================================================\n');

  // 1. Pack all packages
  console.log('=== Stage 1: Packing monorepo canary tarballs ===');
  const tarballDir = path.join(os.tmpdir(), `appspine-pl506-tarballs-${Date.now()}`);
  ...
  // 2. Configure chat overrides
  console.log('\n=== Stage 2: Applying tarball overrides to chat ===');
  applyTarballOverrides(chatRoot, tarballs);

  // 3. Install dependencies in chat
  console.log('\n=== Stage 3: Installing canary packages into chat ===');
  run('pnpm', ['install', '--no-frozen-lockfile'], { cwd: chatRoot });

  // 4. Regenerate artifacts via appspine build
  console.log('\n=== Stage 4: Regenerating plugin artifacts (appspine build) ===');
  run('pnpm', ['-C', 'backend', 'appspine:build'], { cwd: chatRoot });

  // 5. Zero-drift check
  console.log('\n=== Stage 5: Verifying zero drift (appspine build --check) ===');
  run('pnpm', ['-C', 'backend', 'appspine:check'], { cwd: chatRoot });
  ...
  const testEnv = {
    ...
    APP_NAME: 'chat',
    CORS_ORIGINS: 'http://localhost:3041',
  };
  run('pnpm', ['-C', 'backend', 'appspine:doctor'], { cwd: chatRoot, env: testEnv });

  // 7. Prisma generate
  console.log('\n=== Stage 7: Prisma generate ===');
  run('pnpm', ['-C', 'backend', 'prisma:generate'], { cwd: chatRoot });

  // 8. Backend build & typecheck
  console.log('\n=== Stage 8: Backend build and typecheck ===');
  run('pnpm', ['-C', 'backend', 'typecheck'], { cwd: chatRoot });
  run('pnpm', ['-C', 'backend', 'build'], { cwd: chatRoot });

  // 9. Backend test suite (Dual-mode verification)
  console.log('\n=== Stage 9: Backend test suite (Dual-mode) ===');
  run('pnpm', ['-C', 'backend', 'test'], { cwd: chatRoot });

  // 10. Frontend build & typecheck
  console.log('\n=== Stage 10: Frontend typecheck ===');
  run('pnpm', ['-C', 'frontend', 'typecheck'], { cwd: chatRoot });

  // 11. Real bootstrap check in Plugin Mode against disposable Postgres + Realtime Shutdown
  console.log(
    '\n=== Stage 11: Real NestJS Bootstrap & Realtime Lifecycle Shutdown in Plugin Mode ===',
  );
  const DB_CONTAINER = `appspine-pl506-db-${Date.now()}`;
  const DB_PORT = 39435;
  const DB_URL = `postgresql://postgres:rehearsal@127.0.0.1:${DB_PORT}/app_db`;
  const backendDir = path.join(chatRoot, 'backend');
  ...
  try {
    ...
    try {
      console.log('Waiting for backend server to listen on :3996...');
      const status = await waitForHttp('http://localhost:3996/', 25000);
      console.log(
        `✓ Backend responded with HTTP ${status} — real app.listen() succeeded in Plugin Mode.`,
      );

      // Verify WebSocket endpoint socket.io polling endpoint
      console.log('Verifying WebSocket polling endpoint on /socket.io/...');
      const wsStatus = await waitForHttp(
        'http://localhost:3996/socket.io/?EIO=4&transport=polling',
        10000,
      );
  ...
  console.log('\n===============================================================');
  console.log('PL5-06: CHAT WAVE A PLUGIN MODE VERIFICATION COMPLETED!');
  console.log('===============================================================\n');
}
