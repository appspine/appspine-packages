...
// Both stubs exist only to keep `@prisma/client` (which has no generated client in this
// workspace) out of the module graph. Nothing under test reads either of them.
...
type LegacyRole = { name: string; permissionPolicy: string; permissions: { permission: string }[] };
...
type LegacyUserRow = {
  ...
  name?: string | null;
  isActive: boolean;
  userRoles?: { role: LegacyRole }[];
};

function strip(user: LegacyUserRow) {
  return {
    id: user.id,
    email: user.email,
    name: user.name ?? null,
...
function withRoles(user: LegacyUserRow) {
  return { ...strip(user), roles: (user.userRoles ?? []).map((entry) => entry.role) };
}

/**
 * Builds the service over the ports it depends on after the 051 split (PL1-12).
 *
 * The three callbacks keep the pre-split spec's shape on purpose - `findUnique` still returns the
 * familiar user row, `create` is still the provisioning call, and `record` is still the audit
 * write - so every behavioural assertion below compares the new wiring against the old behaviour
 * rather than against a freshly invented expectation.
 *
 * What is genuinely new is `OidcIdentityService`: identity is keyed on (issuer, subject), and
 * these tests run against "no mapping exists yet", which is the state of every database on the day
 * this package ships.
 */
function createService(
  ...
  const identityStore = {
    ...
    async findWithRolesById() {
      const user = await row();
      return user ? withRoles(user) : null;
    },
    async findWithRolesByEmail() {
      const user = await row();
      return user ? withRoles(user) : null;
    },
    async create(input: { email: string; name?: string | null }) {
      const created = (await create(input as never)) as { id: string };
      return {
        id: created.id,
        email: input.email,
  ...
  const oidcIdentity = new OidcIdentityService(
    prisma as never,
  ...
  it('validates an OIDC token signature and attaches local RBAC context', async () => {
    ...
    const token = jwt.sign(
      ...
      privateKey,
      {
        ...
        // Identity is keyed on (issuer, subject) since 051 PL0-04 §4; a real IdP token always
        // carries both, and one that does not is refused rather than falling back to email.
        subject: 'oidc-sub-1',
      },
    );
...
describe('JwtVerifierService.buildOidcJwtUser JIT provisioning', () => {
  ...
  it('auto-creates a local User with default role when none exists locally', async () => {
    const findUnique = vi
      ...
      .mockResolvedValue(provisionedUser); // every lookup after provisioning sees the new row
  ...
  it('recovers from a concurrent first-login race by re-fetching instead of throwing', async () => {
    const findUnique = vi
      ...
      .mockResolvedValue(provisionedUser); // re-fetch after losing the create race
...
describe('JwtVerifierService.mapVerifiedIdentityToLocalPrincipal / findLocalPrincipalByVerifiedEmail', () => {
  ...
  it('mapVerifiedIdentityToLocalPrincipal audits the account it JIT-provisions', async () => {
    // The delegated path is the outermost trust boundary: another App's token makes an account
    // exist here. The pre-split `provisionOidcUser()` wrote this record and the 051 split dropped
    // it silently, because nothing asserted on it. This is that assertion.
    const findUnique = vi
      .fn()
      .mockResolvedValueOnce(null)
      .mockResolvedValueOnce({ ...existingUser, id: 'user-new', email: 'new@example.com' });
    ...
    await service.mapVerifiedIdentityToLocalPrincipal('new@example.com', 'New');
  ...
  it('mapVerifiedIdentityToLocalPrincipal still logs in when the audit write fails', async () => {
    // Best-effort, as before the split: an audit outage must not turn a valid delegated login into
    // a failure. The interactive path is transactional instead — there the audit record and the
    // mapping row it describes must land together.
    const findUnique = vi
      .fn()
      .mockResolvedValueOnce(null)
      .mockResolvedValueOnce({ ...existingUser, id: 'user-new', email: 'new@example.com' });
    const create = vi.fn().mockResolvedValue({ id: 'user-new' });
    const record = vi.fn().mockRejectedValue(new Error('audit sink down'));
    const service = createService(findUnique, create, record);
  ...
  it('mapVerifiedIdentityToLocalPrincipal survives a concurrent first login for the same email', async () => {
    // Both requests saw "no local account" and both called create(); the loser must not fail a
    // login that is, from the user's point of view, perfectly valid.
    const winner = { ...existingUser, id: 'user-winner', email: 'new@example.com' };
    const findUnique = vi.fn().mockResolvedValueOnce(null).mockResolvedValue(winner);
    const create = vi.fn().mockRejectedValue(new Error('duplicate email'));
    const service = createService(findUnique, create);

    const result = await service.mapVerifiedIdentityToLocalPrincipal('new@example.com', 'New');

    expect(result.sub).toBe('user-winner');
  });
