#!/usr/bin/env node
/**
 * PL5-05 — Calendar Wave A Canary Plugin Mode Verification Script
 *
 * Validates:
 *  1. Builds and packs all 22 monorepo packages into clean canary tarballs.
 *  2. Installs canary tarballs into calendar via concrete tarball overrides.
...
const calendarRoot = path.resolve('D:/Source/Private/appspine/calendar');
...
async function main() {
  console.log('===============================================================');
  console.log('PL5-05: Calendar Wave A Plugin Mode End-to-End Verification');
  console.log('===============================================================\n');

  // 1. Pack all packages
  console.log('=== Stage 1: Packing monorepo canary tarballs ===');
  const tarballDir = path.join(os.tmpdir(), `appspine-pl505-tarballs-${Date.now()}`);
  ...
  // 2. Configure calendar overrides
  console.log('\n=== Stage 2: Applying tarball overrides to calendar ===');
  applyTarballOverrides(calendarRoot, tarballs);

  // 3. Install dependencies in calendar
  console.log('\n=== Stage 3: Installing canary packages into calendar ===');
  run('pnpm', ['install', '--no-frozen-lockfile'], { cwd: calendarRoot });

  // 4. Regenerate artifacts via appspine build
  console.log('\n=== Stage 4: Regenerating plugin artifacts (appspine build) ===');
  run('pnpm', ['-C', 'backend', 'appspine:build'], { cwd: calendarRoot });

  // 5. Zero-drift check
  console.log('\n=== Stage 5: Verifying zero drift (appspine build --check) ===');
  run('pnpm', ['-C', 'backend', 'appspine:check'], { cwd: calendarRoot });
  ...
  const testEnv = {
    ...
    OIDC_AUDIENCE: 'calendar',
    OIDC_JWKS_URL: 'https://issuer.example/realms/test/protocol/openid-connect/certs',
    APP_NAME: 'calendar',
  };
  run('pnpm', ['-C', 'backend', 'appspine:doctor'], { cwd: calendarRoot, env: testEnv });

  // 7. Prisma generate
  console.log('\n=== Stage 7: Prisma generate ===');
  run('pnpm', ['-C', 'backend', 'prisma:generate'], { cwd: calendarRoot });

  // 8. Backend build & typecheck
  console.log('\n=== Stage 8: Backend build and typecheck ===');
  run('pnpm', ['-C', 'backend', 'typecheck'], { cwd: calendarRoot });
  run('pnpm', ['-C', 'backend', 'build'], { cwd: calendarRoot });

  // 9. Backend test suite (Dual-mode verification)
  console.log('\n=== Stage 9: Backend test suite (Dual-mode) ===');
  run('pnpm', ['-C', 'backend', 'test'], { cwd: calendarRoot });

  // 10. Frontend build & typecheck
  console.log('\n=== Stage 10: Frontend typecheck ===');
  run('pnpm', ['-C', 'frontend', 'typecheck'], { cwd: calendarRoot });
  ...
  const DB_CONTAINER = `appspine-pl505-db-${Date.now()}`;
  const DB_PORT = 39434;
  const DB_URL = `postgresql://postgres:rehearsal@127.0.0.1:${DB_PORT}/app_db`;
  const backendDir = path.join(calendarRoot, 'backend');
  ...
  try {
    ...
    try {
      console.log('Waiting for backend server to listen on :3997...');
      const status = await waitForHttp('http://localhost:3997/', 25000);
      console.log(
  ...
  console.log('\n===============================================================');
  console.log('PL5-05: CALENDAR WAVE A PLUGIN MODE VERIFICATION COMPLETED!');
  console.log('===============================================================\n');
}
