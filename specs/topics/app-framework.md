...
- [x] `appspine-app-template` 內預先裝好 `@appspine/*` 共用套件、接上最小可運行範例 wiring。八個套件（`common`/`auth`/`rbac`/`m2m-api-key`/`audit-log`/`health-check`/`metadata-schema`/`mcp-server`）從既有 `auranest` 專案 `packages/@auranest/backend-core` 盤點重用後實作完成、發布到 GitHub Packages，並接進 `appspine-app-template`，實際跑過 migration/seed/開機/API 驗證。詳見 [003-shared-package-reuse-plan.md](../_archive/legacy-knowledge/decisions/003-shared-package-reuse-plan.md)。
