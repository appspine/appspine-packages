...
# 051 PL5-05 — Calendar Wave A Canary Rollout 報告

> Task：`PL5-05`（見 [051 拆解 §9](../decisions/051-plugin-platform-engineering-task-breakdown.md#9-phase-5--release全-app-rollout-與-transition-window)）。  
...
> 依賴：[PL5-04](051-pl5-04-wiki-canary-rollout.md)（已完成）。  
> Calendar 目標分支：`051-pl5-05-calendar-wave-a`（Commit: `3f1c42d`）。  
> 驗證腳本：[`scripts/051-pl5-05-calendar-wave-a.mjs`](../../scripts/051-pl5-05-calendar-wave-a.mjs)。  

---

## 1. 執行摘要 (Executive Summary)

依據 [051 計畫書 §3、§7、§9](../decisions/051-plugin-platform-engineering-plan.md)，已於 `calendar` 業務倉庫完成 Wave A 的 Canary Plugin Mode Rollout：
...
3. **App-Owned Modules 保留與 DI 隔離 (App-Owned Modules Preservation)**：
   - 保留 Calendar 專屬業務模組（`CalendarsModule`、`EventsModule`），並匯入 `AppspineAuthInfrastructureModule`，確保 Controller 在非全域 Auth 的 Plugin Mode 下能正確解析 Guards。
...
5. **11 階段端到端驗證與真實啟動 (End-to-End Verification)**：
   - 通過 Zero-Drift 檢查、`appspine doctor`（10 enabled, 0 findings）、Prisma generation、全庫 typecheck、雙模式單元測試（22/22 tests pass）、以及對獨立 Disposable Postgres 的**真實 NestJS `app.listen()` HTTP 404 開機回應驗證**。

---

## 2. 變更檔案清單 (Modified Files in Calendar)

| 檔案路徑 | 變更說明 |
|---|---|
| `backend/appspine.plugins.json` | 宣告採用 `@appspine/preset-standard` 預設集。 |
| `backend/src/appspine.config.ts` | 宣告 calendar runtime 設定與 `appspine.prisma` host capability。 |
...
| `backend/src/calendars/calendars.module.ts` | 匯入 `AppspineAuthInfrastructureModule`。 |
| `backend/src/events/events.module.ts` | 匯入 `AppspineAuthInfrastructureModule`。 |
| `backend/package.json` | 升級依賴至 Canary 版本，補齊 `date-fns`、`date-fns-tz`、plugin CLI 與 testkit。 |
| `frontend/package.json` | 升級前端依賴至 Canary 版本，補齊 `@fullcalendar/*` 與 `date-fns-tz`。 |
...
執行驗證腳本 [`scripts/051-pl5-05-calendar-wave-a.mjs`](../../scripts/051-pl5-05-calendar-wave-a.mjs)：
...
Tests       22 passed (22) (含 Plugin Mode 預設與 Legacy Mode 測試)
...
Starting disposable postgres container appspine-pl505-db-...
...
===============================================================
PL5-05: CALENDAR WAVE A PLUGIN MODE VERIFICATION COMPLETED!
===============================================================
```

---

## 4. 下一步前置條件檢查 (Prerequisites for PL5-06 Chat)

- [x] Calendar Wave A Rollout 已順利完成並通過全套驗證與 Commit（`3f1c42d`）。
- [x] 可以正式進入 **PL5-06**（`chat` Wave A Rollout，專門驗證 Realtime/WebSocket Lifecycle 與 Shutdown Hooks）。
...
| **Substitution reason** | 當前環境無獨立 Terra session；由 Gemini 兼任 Calendar 倉庫的 Dual-Mode 架構接入、依賴升級、App-owned DI 修復與真實驗證，後續由 Claude 進行獨立審核。 |
...
| **Evidence** | Calendar 分支 `051-pl5-05-calendar-wave-a` (Commit `3f1c42d`)、本報告 `051-pl5-05-calendar-wave-a-rollout.md`、腳本 `scripts/051-pl5-05-calendar-wave-a.mjs`。 |
